                    ...
                    if (jQuery('#cocobasic_shortcodes_popup_holder').is(":visible")) {
                        ...
                        jQuery('#cocobasic_shortcodes_popup_holder').load(url + '/cocobasic_shortcodes_popup.php#cocobasic_shortcodes_popup', function () {
                            ...
                            jQuery("#cocobasic_info_text").click(function () {
                                var shortcode = '[info title="PROJECT"]Corporate Branding[/info]<br/>[info title="CLIENT"]Envato Studio[/info]';
                                tinyMCE.activeEditor.execCommand('mceInsertContent', 0, shortcode);
                                jQuery("#cocobasic_shortcodes_popup_holder").hide();
                            });

                            jQuery("#cocobasic_skills").click(function () {
                                var shortcode = '[skills title="Photoshop" percent="75%"]<br/>[skills title="Illustrator" percent="55%"]<br/>[skills title="Html/Css" percent="85%"]<br/>[skills title="Super Mario" percent="100%"]';
                                tinyMCE.activeEditor.execCommand('mceInsertContent', 0, shortcode);
                                jQuery("#cocobasic_shortcodes_popup_holder").hide();
                            });

                            jQuery("#cocobasic_text_slider").click(function () {
                                var shortcode = '[text_slider pagination="dots"]<br/>[text_slide]The goal of a designer is to listen, observe, synthesize, and glean insights that enable him or her to make the invisible visible[/text_slide]<br/>[text_slide]All architecture is shelter, all great architecture is the design of space that contains, cuddles, exalts, or stimulates the persons in that space[/text_slide]<br/>[text_slide]As an architect, you design for the present, with an awareness of the past, for a future which is essentially unknown.[/text_slide]<br/>[/text_slider]';
                                tinyMCE.activeEditor.execCommand('mceInsertContent', 0, shortcode);
                                jQuery("#cocobasic_shortcodes_popup_holder").hide();
                            });
                            ...
                            jQuery("#cocobasic_team_holder").click(function () {
                                var shortcode = '[team]<br/><br/>[/team]';
                                tinyMCE.activeEditor.execCommand('mceInsertContent', 0, shortcode);
                                jQuery("#cocobasic_shortcodes_popup_holder").hide();
                            });

                            jQuery("#cocobasic_team_member").click(function () {
                                addTeamMemberHtml();
                            });
...
//  TEAM MEMBER  //
var addTeamMemberHtml = function () {
    var params = {
        'name': 'teamMember'
    };
    jQuery.ajax({
        ...
        form.find('#submit_shortcode').click(function () {
            ...
            for (var index in options) {
                var value = table.find('#shortcode_teamMember_' + index).val();
                if (value != '')
                {
                    shortcode += ' ' + index + '="' + value + '"';
                }
        ...
        W = W - 80;
        H = H - 84;
        tb_show('Team Member Shortcode', '#TB_inline?width=' + W + '&height=' + H + '&inlineId=shortcodes_teamMember_form');
    });
};
