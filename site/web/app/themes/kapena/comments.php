...
<div id="comments" class="comments-holder">	
    ...
    if (comments_open()) :
        ...
        comment_form(
                array('fields' => array(
                        'author' => '<input id="author" name="author" type="text" placeholder="' . esc_html__('NAME', 'kapena-wp') . '" value="' . esc_attr($commenter['comment_author']) . '" size="20"' . $aria_req . ' />',
                        'email' => '<input id="email" name="email" type="text" placeholder="' . esc_html__('EMAIL', 'kapena-wp') . '" value="' . esc_attr($commenter['comment_author_email']) . '" size="20"' . $aria_req . ' />'
                    ),
                    'comment_field' => '<textarea id="comment" placeholder="' . esc_html__('COMMENT', 'kapena-wp') . '" name="comment" cols="45" rows="12" aria-required="true"></textarea>',
                    'title_reply' => '',
                    'comment_notes_before' => '',
                    'comment_notes_after' => '',
                    'label_submit' => esc_html__('PUBLISH COMMENT', 'kapena-wp')));
        echo '</div></div>';
    endif;
